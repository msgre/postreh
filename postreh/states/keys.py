    ...
    if keys_player >= 0 and keys_player not in disqualified:
        ...
        if tick < context["round_timeout"]:
            ...
            if len(disqualified) == PLAYERS:
                disqualified = []
                print(
                    "KEYS: Vsichni diskvalifikovani, co je toto za partu? Reset disqualified"
                )
            else:
        ...
        if context["score"][keys_player] >= SCORES_PER_PLAYER:
            ...
            context["winner"] = keys_player
            LED_NUMBERS.set(
                keys_player * SCORES_PER_PLAYER + SCORES_PER_PLAYER - 1, WHITE
            )
            LED_NUMBERS.show()
        ...
        print(
            "KEYS: Hrac {} ziskava dalsi bod, celkem ma uz {}".format(
                keys_player, context["score"][keys_player]
            )
        )
        return states.WIN

    if timeout_tick and tick > timeout_tick:
        # nikdo nereaguje
